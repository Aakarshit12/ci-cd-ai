...
def register_service(service: ServiceCreate, db: Session = Depends(get_db)):
    db_service = (
        ...
        .filter(RegisteredService.name == service.name)
        .first()
    )
    if db_service:
        raise HTTPException(
...
def remove_service(service_id: UUID, db: Session = Depends(get_db)):
    db_service = (
        db.query(RegisteredService).filter(RegisteredService.id == service_id).first()
    )
    if not db_service:
...
def update_service(
    service_id: UUID, service_update: ServiceUpdate, db: Session = Depends(get_db)
):
    db_service = (
        db.query(RegisteredService).filter(RegisteredService.id == service_id).first()
    )
