...
async def check_sliding_window(limit_type: str, identifier: str, limit: int, window: int = 60) -> Tuple[bool, int]:
    """
...
async def check_rate_limits(ip: str, api_key: str = None) -> Tuple[bool, str, int]:
    ...
    """
    # 1. Global limit
    is_allowed, retry_after = await check_sliding_window("global", "all", RATE_LIMIT_GLOBAL)
    if not is_allowed:
    ...
    # 3. API Key limit
    if api_key:
        is_allowed, retry_after = await check_sliding_window("key", api_key, RATE_LIMIT_PER_KEY)
        if not is_allowed:
